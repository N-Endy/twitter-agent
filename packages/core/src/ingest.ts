...
export async function ingestUrlSource(url: string) {
...
export async function ingestRssSource(url: string) {
...
export async function ingestXPostSource(url: string) {
  ...
  const data = response.data.data as { text?: string; id?: string } | undefined;
...
export async function ingestXAccountSource(username: string) {
  const env = getEnv();
  ...
  const data = (posts.data.data as Array<{ id?: string; text?: string }> | undefined) ?? [];
